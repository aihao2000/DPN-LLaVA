...
class S2FStitchAlignModuleV2(nn.Module):
    def __init__(self, fast_vision_dim, slow_vision_dim, zero_init=True):
        ...
        if not os.getenv('TRAIN_ROUTER_FROM_PRETRAINED'):
...
class MultiPathCLIPVisionTower(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self,'prepooling') and self.prepooling:
            ori_shape = y.shape
            height = width = int(math.sqrt(y.shape[1]))
            y = y.view(y.shape[0], height, width, y.shape[-1])
            y = y.permute(0,3,1,2)
            y = F.avg_pool2d(y, kernel_size=(2,2),stride=(2,2))
            y = y.permute(0,2,3,1)
            y = y.view(ori_shape[0],-1,ori_shape[-1])
        return y
