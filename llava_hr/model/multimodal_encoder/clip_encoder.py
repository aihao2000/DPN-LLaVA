...
class CLIPVisionTower(nn.Module):
    ...
    def forward(self, images):
        ...
        if hasattr(self,'prepooling') and self.prepooling:
            ori_shape = image_features.shape
            height = width = int(math.sqrt(image_features.shape[1]))
            image_features = image_features.view(image_features.shape[0], height, width, image_features.shape[-1])
            image_features = image_features.permute(0,3,1,2)
            image_features = F.avg_pool2d(image_features, kernel_size=(2,2),stride=(2,2))
            image_features = image_features.permute(0,2,3,1)
            image_features = image_features.view(ori_shape[0],-1,ori_shape[-1])
