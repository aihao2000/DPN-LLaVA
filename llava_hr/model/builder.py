...
def load_pretrained_model(
    ...
    if "llava" in model_name.lower():
        # Load LLaVA model
        if (
            "lora" in model_name.lower() or "lavin" in model_name.lower()
        ) and model_base is None:
            warnings.warn(
        ...
        if (
            ...
            additional_weight = (
                "non_lavin_trainables.bin"
                if enable_lavin
                else "non_lora_trainables.bin"
            )
            if os.path.exists(os.path.join(model_path, additional_weight)):
                non_lora_trainables = torch.load(
                    os.path.join(model_path, additional_weight), map_location="cpu"
                )
                if enable_lavin:
                    lavin_weight = torch.load(
                        os.path.join(model_path, "lavin_trainables.bin"),
                        map_location="cpu",
                    )
            ...
            if "mpt" in model_name.lower():
                ...
                model = LlavaMPTForCausalLM.from_pretrained(
                    model_path, low_cpu_mem_usage=low_cpu_mem_usage, **kwargs
                )
            else:
