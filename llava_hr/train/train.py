...
import random
from llava_hr.hack.vision_token_purning_handler import hack_llava
...
class ModelArguments:
    ...
    vision_tower_slow: Optional[str] = field(
        default="convnext_large_mlp.clip_laion2b_ft_320"
    )
    pooling_type: str = "adaptive"
    pooling_function: str = "max"
    pooling_location: str = "post"
    router_version: str = "v1"
    prepooling: bool = field(default=False)
...
def add_adversarial_example(ques, refs, ans, template_, neg_prob=0.3):
    ...
    if random.random() <= neg_prob:
        q0 = (
            template_inst.replace("<ref>", refs)
...
def add_robust_tokens(source, adversarial_prob=0.3):
    ...
    for j in range(len(ques)):
        ...
        if random.random() > adversarial_prob or len(refs[j]) == 0:
            ...
            ques_, ans_ = add_adversarial_example(
                ques[j], refs[j], ans[j], template_, neg_prob=0.7
            )
...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    for source in sources:
        is_multimodal_source = any(
            [DEFAULT_IMAGE_TOKEN in sentence["value"] for sentence in source]
        )
        for i, sentence in enumerate(source):
            ...
            if is_multimodal_source and data_args.use_special_token and i % 2 == 0:
                sentence["value"] += "<ROUTER>"
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if data_args.vqa_only:
            self.list_data_dict = [
                item for item in list_data_dict if "image" in item.keys()
            ]
        ...
        if self.data_args.lavin_enable:
            data_dict["input_ids"] = torch.cat(
                [
                    ...
                    torch.tensor(
                        [self.tokenizer.convert_tokens_to_ids(modality_token)],
                        dtype=torch.long,
                    ),
                    data_dict["input_ids"][1:],
                ],
            ...
            data_dict["labels"] = torch.cat(
                [
                    data_dict["labels"][:1],
                    torch.tensor([IGNORE_INDEX], dtype=torch.long),
                    data_dict["labels"][1:],
                ],
                0,
            )
...
def train():
    ...
    if training_args.lavin_enable:
        ...
        if training_args.pretrain_lavin_adapter is not None:
            non_lavin_trainables = torch.load(
                os.path.join(
                    training_args.pretrain_lavin_adapter, "non_lavin_trainables.bin"
                ),
                map_location="cpu",
            )
            lavin_trainables = torch.load(
                os.path.join(
                    training_args.pretrain_lavin_adapter, "lavin_trainables.bin"
                ),
                map_location="cpu",
            )
            ...
            lavin_trainables = {
                ...
                for k, v in lavin_trainables.items()
            }
    ...
    hack_llava(
        ...
        pooling_function=model_args.pooling_function,
        pooling_location=model_args.pooling_location,
        prepooling=model_args.prepooling,
        use_special_token=data_args.use_special_token,
        log_routing_statistics=False,
        split_autoregressive_loss=training_args.split_autoregressive_loss,
        use_router_contrastive_loss=training_args.use_router_contrastive_loss,
        router_version=model_args.router_version,
    )
    ...
    if training_args.lora_enable:
        ...
        word_embedding_state_dict = {
            k: maybe_zero_3(t, ignore_status=True)
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            ...
            torch.save(
                state_dict,
                os.path.join(training_args.output_dir, "lavin_trainables.bin"),
            )
            torch.save(
                non_lora_state_dict,
                os.path.join(training_args.output_dir, "non_lavin_trainables.bin"),
            )
    else:
