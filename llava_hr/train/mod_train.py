...
from gamma_mod.constants import (
    IGNORE_INDEX,
    IMAGE_TOKEN_INDEX,
    DEFAULT_IMAGE_TOKEN,
...
def save_model_in_parts(state_dict, output_dir, num_parts=5):
    ...
    for i, key in enumerate(keys):
        ...
        total_size += (
            torch.prod(torch.tensor(state_dict[key].size())).item()
            * state_dict[key].element_size()
        )

    # Save each part
    for i in range(1, num_parts + 1):
        part_file = os.path.join(
            output_dir, f"pytorch_model-{i:04d}-of-{num_parts:05d}.bin"
        )
    ...
    with open(
        os.path.join(output_dir, "pytorch_model.bin.index.json"), "w"
    ) as index_file:
...
def preprocess_llama_3(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if has_image:
                round_len = len(tokenizer_image_token(rou, tokenizer))
                instruction_len = (
                    len(tokenizer_image_token(parts[0], tokenizer)) - offset
                )
            else:
                round_len = len(tokenizer(rou).input_ids)
            ...
            cur_len += round_len + (
                1 - offset
            )  # starting from index 0, then cur_len will not cover eos token
    ...
    if input_ids[0][0] != tokenizer.bos_token_id:
        input_ids = [
            torch.cat([torch.LongTensor([tokenizer.bos_token_id]), i])
            for i in input_ids
        ]
